...
[source](https://github.com/harrrshall/kriti) · [architecture](https://github.com/harrrshall/kriti/blob/main/architecture.md) · [training](https://github.com/harrrshall/kriti/blob/main/training.md) · [benchmark data](https://huggingface.co/harrrshall/kriti/blob/main/benchmark.json)
...
pi wer is punctuation-insensitive word error rate. exact primary error ratios share a rank, and raw wer never breaks that tie. the fine-tuned nepali qwen model is included at rank 4. inspect the [exact model revisions and replicate hashes](https://huggingface.co/harrrshall/kriti/blob/main/benchmark.json).
...
the rnnt creates the transcript while the tiny acoustic head decides whether terminal devanagari danda is needed. read the [deep architecture note](https://github.com/harrrshall/kriti/blob/main/architecture.md).
...
model construction and evaluation used nvidia h200 gpu nodes. encoder feature extraction used one gpu, while the 1,025-parameter logistic fit ran on cpu. the full detail is in the [training recipe](https://github.com/harrrshall/kriti/blob/main/training.md).
...
audio is normalized to mono 16 khz, text uses unicode nfc and conservative space cleanup, and speaker or recording groups stay within one split. obtain every source under its original terms. read the [data note](https://github.com/harrrshall/kriti/blob/main/data.md).
...
the public evaluator checks the kriti view hash, row count, and source counts. this command also fixes the benchmark batch size at 32:
